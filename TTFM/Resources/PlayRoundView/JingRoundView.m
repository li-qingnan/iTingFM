...
-(void) initJingRound
{
    ...
    UIImage *stateImage;
    if (self.isPlay) {
        stateImage = [UIImage imageNamed:@"pauseMain.png"];
    }else{        
        stateImage = [UIImage imageNamed:@"playMain.png"];
    }
    ...
    CGContextRef context = CGBitmapContextCreate(nil,self.frame.size.width,self.frame.size.height,8,0, colorSpace,(CGBitmapInfo)kCGImageAlphaPremultipliedLast);
...
-(void) startRotation
{
    ...
    //set ImgView
    self.playStateView.image = [UIImage imageNamed:@"pauseMain.png"];
...
{
    //set ImgView
    self.playStateView.image = [UIImage imageNamed:@"playMain.png"];
    self.playStateView.alpha = 0;
    [UIView animateWithDuration:1.0 delay:0 options:UIViewAnimationOptionCurveEaseInOut
                     animations:^{
                         self.playStateView.alpha = 1;
